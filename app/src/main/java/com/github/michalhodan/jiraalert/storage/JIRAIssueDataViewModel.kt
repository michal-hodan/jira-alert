...
class JIRAIssueDataViewModel(private val jira: JIRA, private val database: Database): ViewModel() {
    ...
    private val issue: HashMap<Int, MutableLiveData<IssueDetailData>> = hashMapOf()

    fun detail(issueId: Int) = issue.getOrPut(issueId) {
        MutableLiveData<IssueDetailData>().apply {
    ...
    private fun load(issueId: Int): IssueDetailData {
        val issue = database.issue().findById(issueId)!!
        return IssueDetailData(
            issue = issue,
            sprint = database.sprint().retrieve(issue.sprintId)!!,
